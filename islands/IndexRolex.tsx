...
function RolexIndex(props: Props) {
    ...
    const setup = ({ rootId, scroll, interval, infinite, images }: PropsI & { images: BannerI[] }) => {
        const ATTRIBUTES = {
        ...
        const updateCurrentIndex = (index: number, images: BannerI[]) => {
            currentIndex = index;
            const color = images[currentIndex]?.action?.color === 'Black' ? 'text-[#452c1e] bg-[#d4d4d44d] hover:bg-[#f3f3f380]' : 'text-white bg-[#7676764d] hover:bg-[#54545480]';
            prev?.setAttribute('data-current-index', currentIndex.toString());
            next?.setAttribute('data-current-index', currentIndex.toString());
            prev?.setAttribute('class', `btn btn-circle w-10 max-h-10 min-h-[40px] border-0 disabled:hidden ${color}`);
            next?.setAttribute('class', `btn btn-circle w-10 max-h-10 min-h-[40px] border-0 disabled:hidden ${color}`);
        ...
        const onClickPrev = () => {
            ...
            const newIndex = isShowingFirst ? items.length - 1 : (pageIndex - 1) * itemsPerPage;
            goToItem(newIndex);
        };
    ...
    function JS({
        ...
        return (
            <script
                src={scriptAsDataURI(setup, { rootId, scroll, interval, infinite, images })}
                defer
            />
        );
    }
    ...
    return (
        <>
            ...
            >
                ...
                <Slider.JS rootId={id} interval={interval && interval * 1e3} images={images} />
            </div>
                ...
                <div class="rolex-container flex justify-between items-center max-md:flex-col">
                    ...
                    <div class="w-full md:w-[50%] md:pr-[8%]">
                        <p class="body20-ligth text-[#212121]">
                            ...
                            , autorizados a vender e fazer a manutenção dos
                            relógios Rolex. Na Dryzun, nos dedicamos a
                            apresentar-lhe toda a coleção Rolex e a acompanhá-lo
                            na escolha de um relógio para a eternidade.
                        </p>
                    </div>
                </div>
            </section>
