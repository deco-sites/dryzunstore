...
function Gallery(props: Props) {
  const { images, screen, hexa, paddingTop, paddingBottom, text, color } = {
    ...DEFAULT_PROPS,
    ...props,
  };
  ...
  return (
    ...
    >
      {images?.map((image, index) => (
        <div
          class={`${screen == "S" ? "rolex-container-3" : "w-full"}`}
          key={index}
        >
          <img
            class="w-full h-auto md:hidden h-[120px] object-cover"
            loading="lazy"
            src={image.mobile}
            width={780}
            height={520}
      ...
      {text &&
        (
          <div
            class="w-full h-full bg-[#0000000f] flex items-center justify-center absolute mx-auto f-rolex text-[14px] md:text-[22px] text-center font-black uppercase tracking-[-2px]"
