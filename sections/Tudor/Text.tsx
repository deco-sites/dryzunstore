...
function Text(props: Props) {
    const { subtitle, title, isColumns, NotisH1, text, screen, hexa, color, paddingTop, paddingBottom } = { ...DEFAULT_PROPS, ...props };
        ...
        ${screen == 'XL' && 'w-full'} ${screen == 'L' && 'rolex-container'} ${screen == 'M' && 'rolex-container-2'}`}>
            <div class={`${isColumns && 'w-full md:w-[40%]'}`}>
                ...
                {NotisH1 ?
                    ...
                    : <h1 class={`${screen == 'S' && 'md:w-[800px] mx-auto'} f-rolex tudorTitle text-center mb-2`}>{title}</h1>
                }
            </div>
            <div class={`${isColumns && 'w-full md:w-[50%]'} ${screen == 'S' && 'md:w-[700px] mx-auto'} f-rolex tudorText`}>
                <p style={{color: `#${color}`}} dangerouslySetInnerHTML={{
                    __html: text,
                }} class={`${screen == 'S' && 'md:w-[800px] mx-auto'} f-rolex tudorText`}></p>
            </div>
        </div>
    )
}
