...
function ProductShelfCollection({ products, description, layout, desktop, mobile, link }: Props) {
    const id = useId();
    ...
    return (
        <div class="w-full container-2 flex flex-col gap-6 lg:py-3 mb-[50px]">
            ...
            <div id={id}
                ...
                <Slider class="carousel carousel-center sm:carousel-end sm:gap-1 row-start-2 row-end-5">
                    {products?.map((product, index) => (
                        <Slider.Item index={index} class={clx("carousel-item", "md:w-1/4", "w-1/2")}>
                            <ProductCard product={product} platform={platform} index={index} />
                        </Slider.Item>
                    ))}
                </Slider>
            ...
            <div class="mx-auto my-0">
                <a href={link} class="border block text-center text-[#243959] text-xs not-italic font-bold leading-[normal] tracking-[1.2px] uppercase px-8 py-4 border-solid border-[#243959] hover:bg-[#243959] hover:text-[#fff] transition-[.3s]">
