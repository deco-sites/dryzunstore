            ...
            <div class="rolex-container-vitrine py-[60px] md:py-[90px]" style={background}>
                ...
                >
                    {products?.map((product, index) => (
                        <div class={`${index < more ? 'block' : 'hidden'} w-[48.5%] md:w-[32%] card-${index}`}>
                            <ProductCard
                                product={product}
                                itemListName={title}
                                index={index}
                    ...
                    {more < maxMore && (
                        <div class="w-full flex items-center justify-center mt-5">
                            <button {...usePartialSection({ props: { more: more + 6 } })} class="btn primary-cta bg-rolex-1 w-fit hover:bg-rolex-1">Ver mais</button>
                        </div>
                    )}
