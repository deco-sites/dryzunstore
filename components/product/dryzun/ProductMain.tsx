...
function ProductMain({ page }: Props) {
  ...
  const impermeabilidade = product?.isVariantOf?.additionalProperty?.find((item: any) =>
    item.name === "Impermeabilidade à Água"
  )?.value;

  const enrolamento = product?.isVariantOf?.additionalProperty?.find((item: any) =>
    item.name === "Coroa de Enrolamento"
  )?.value;

  const bracelete = product?.isVariantOf?.additionalProperty?.find((item: any) =>
    item.name === "Bracelete"
  )?.value;
          ...
          <div class="w-full flex flex-wrap justify-start items-center py-2">
            <span class="flex items-center mr-10 max-md:mb-4 text-[13px] not-italic font-semibold leading-[normal] text-[#597CB2]">
              <svg class="mr-1" width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0.5 1.54248C0.5 0.72998 1.15625 0.0424805 2 0.0737305H6.65625C7.1875 0.0737305 7.6875 0.26123 8.0625 0.63623L13.5625 6.13623C14.3438 6.91748 14.3438 8.19873 13.5625 8.97998L9.40625 13.1362C8.625 13.9175 7.34375 13.9175 6.5625 13.1362L1.0625 7.63623C0.6875 7.26123 0.5 6.76123 0.5 6.22998V1.54248ZM4 2.54248C3.4375 2.54248 3 3.01123 3 3.54248C3 4.10498 3.4375 4.54248 4 4.54248C4.53125 4.54248 5 4.10498 5 3.54248C5 3.01123 4.53125 2.54248 4 2.54248Z" fill="#597CB2" />
              </svg>
