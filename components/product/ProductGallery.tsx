...
function ProductGallery({ products, pageInfo, layout, offset, url }: Props) {
    ...
    return (<div class={`grid ${mobile} gap-2 items-center ${desktop} sm:gap-0`}>
      ...
      {products?.map((product, index) => (<ProductCard key={`product-card-${product.productID}`} product={product} preload={index === 0} index={offset + index} platform={platform}/>))}
